...
void print_nodes(int last){

    for (int i = 0; i < last; i++){
        printf("N:%d, U:%d, D:%d, I;%d, C:%d\n", i,nd[i].up,nd[i].down,nd[i].itm,nd[i].color);
    }
}
...
void print_items(int last){
    for(int i = 0; i < last; i++){
        printf("I:%d, N:%d, P:%d, B:%d, S:%d\n", i,cl[i].next,cl[i].prev,cl[i].bound,cl[i].slack);
    }
}
/*:11*//*12:*/
#line 381 "dlx3.web"
...
#line 635 "dlx3.web"
    /*
    print_items(last_itm);
    printf("\n\n");
    print_nodes(last_node + 2);
     */
level= 0;
