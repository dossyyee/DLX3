...
void cover(int c,int deact){
    ...
    updates++;
    for(rr= nd[c].down;rr>=last_itm;rr= nd[rr].down)
        for(nn= rr+1;nn!=rr;){
            if(nd[nn].color>=0){
                uu= nd[nn].up;
                dd= nd[nn].down;
...
void purify(int p){
    ...
    nd[cc].color= x;
    cleansings++;
    for(rr= nd[cc].down;rr>=last_itm;rr= nd[rr].down){
        if(nd[rr].color!=x){
            for(nn= rr+1;nn!=rr;){
                uu= nd[nn].up;
                dd= nd[nn].down;
        ...
        }else if(rr!=p)cleansings++,nd[rr].color= -1;
    }
}
...
void unpurify(int p){
    ...
    cc= nd[p].itm;
    x= nd[p].color;
    for(rr= nd[cc].up;rr>=last_itm;rr= nd[rr].up){
        if(nd[rr].color<0)nd[rr].color= x;
        else if(rr!=p){
            for(nn= rr-1;nn!=rr;){
                uu= nd[nn].up;
                dd= nd[nn].down;
...
void untweak(int c,int x,int unblock){
    ...
    z= nd[c].down;
    nd[c].down= x;
    for(rr= x,k= 0,qq= c;rr!=z;qq= rr,rr= nd[rr].down){
        nd[rr].up= qq;
        k++;
